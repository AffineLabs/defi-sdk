...
import { EmergencyWithdrawalQueueEnqueueEvent, EmergencyWithdrawalQueueDequeueEvent } from "../typechain/src/polygon/EmergencyWithdrawalQueue"
import { AlpineProduct, DryRunReceipt, FullTxReceipt, TokenInfo } from "./types";

export async function getUserEmergencyWithdrawalQueueRequests(product: AlpineProduct): 
...
export async function vaultWithdrawableAssetAmount(product: AlpineProduct): 
	Promise<number> {
...
export async function txHasEnqueueEvent(txReceipt: FullTxReceipt): 
	Promise<boolean> {
    const txnReceipt = await PROVIDER.getTransactionReceipt(txReceipt.txnHash)
    let eventABI = [ "event EmergencyWithdrawalQueueEnqueue(uint256 indexed pos, address indexed owner, address indexed receiver, uint256 shares)" ];
...
export async function getEmergencyWithdrawalQueueTransfers(product: AlpineProduct): 
