...
export async function blockchainCall(
  ...
  if (simulate) {
    ...
    const cost = gasEstimate.mul(gasPrice.mul(4).div(3));
  ...
  const tx: TransactionResponse = await contract[method].apply(null, args.concat({ gasPrice: gasPrice.mul(4).div(3) }));
